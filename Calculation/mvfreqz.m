function [S,h,PDC,COH,DTF,DC,pCOH,dDTF,ffDTF, pCOH2, PDCF, coh,GGC,Af,GPDC,AS]=mvfreqz(B,A,C,N,Fs)
% MVFREQZ multivariate frequency response
% [S,h,PDC,COH,DTF,DC,pCOH,dDTF,ffDTF,pCOH2,PDCF,coh,GGC,Af,GPDC,AS] = mvfreqz(B,A,C,f,Fs)
% [...]  = mvfreqz(B,A,C,N,Fs)
%  
% INPUT: 
% ======= 
...
% Huifang Wang, June 4, 2012, 1. added DC in [2]; 2. modified DC as AS;  
...
dc2 = diag(C).^(1/2); 
dcS=diag(C);
dcSigmajj=reshape(repmat(dcS,1,N),1,K1,N);
%%%%% directed transfer function

for k1=1:K1;
        DEN=sum(abs(h(k1,:,:)).^2,2);
        dcS= sum(dcSigmajj.*abs(h(k1,:,:)).^2,2);
        for k2=1:K2;
                ...
                DC(k1,k2,:)=dc2(k2).*h(k1,k2,:)./sqrt(dcS);
