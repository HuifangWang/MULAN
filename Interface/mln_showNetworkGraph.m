...
LineWidth_full=2;
size_arrow=LineWidth_full*110;

radBigCircle = 180;
if Nchannel>18
radiuscircle=radBigCircle*2*pi/(Nchannel+1)/2.5;
else
    radiuscircle=40;
end
...
%drawing  and getting coordinates of network nods
% circles = zeros(Nchannel, 2);
% 
 THETA=linspace(0,2*pi,Nchannel+1);
 THETA = THETA(1:end-1);
% RHO=ones(1,Nchannel)*radBigCircle;
...
set(gca, 'XLim', [-xRange xRange], 'YLim', [-xRange xRange], 'XTick',[],'YTick',[]); 
axis equal;
axis manual;
hold all;

%writing brain structures names
RHO=ones(1,Nchannel)*(radBigCircle+radiuscircle);
[X, Y] = pol2cart(THETA,RHO);
[TX,TY]=pol2cart(THETA,RHO+0.5*radiuscircle);
texts(:, 1) = TX;
texts(:, 2) = TY;
circles(:, 1) = X;
circles(:, 2) = Y;
ThresholdcnM=zeros(Nchannel);
[irow,icol]=find(abs(cnM)>=thresholdnw);
for i=1:length(irow)
    ThresholdcnM(irow(i),icol(i))=cnM(irow(i),icol(i));
end

% find reciprocal edges
temp = ThresholdcnM.*ThresholdcnM';
[toinx,frominx] = find(temp);
R = [toinx frominx];
for i=1:length(toinx),
    xf = X(frominx(i)); yf = Y(frominx(i));
    xt = X(toinx(i)); yt = Y(toinx(i));
    strength=abs(ThresholdcnM(toinx(i),frominx(i)));
    if strength>=thresholdnw
    h = line([xf xt],[yf yt]);
    set(h,'LineWidth',strength.*LineWidth_full);
...
for i=1:length(toinx),
    ...
    xt = X(toinx(i)); yt = Y(toinx(i));
    h = line([xf xt],[yf yt]);
    set(h,'LineWidth',abs(ThresholdcnM(toinx(i),frominx(i))).*LineWidth_full);
    set(h,'Color',[0 1 0]); % green for non-recip edges
    arrowh([xf (xf+xt)/2 ],[yf (yf+yt)/2],'g',size_arrow,50);
end

% label nodes

for i=1:Nchannel
    circle([circles(i,1), circles(i,2)],radiuscircle);
    if(isempty(nodenames))
        %ht=text(texts(i,1), texts(i,2), num2str(i),'HorizontalAlignment','center','VerticalAlignment','middle');
        ht=text(circles(i,1), circles(i,2), num2str(i),'HorizontalAlignment','center','VerticalAlignment','middle');
    else
        if cos(THETA(i))>=0
            ht=text(texts(i,1), texts(i,2), nodenames{i},'HorizontalAlignment','left','VerticalAlignment','middle');
        else
            ht=text(texts(i,1), texts(i,2), nodenames{i},'HorizontalAlignment','right','VerticalAlignment','middle');
        end
    end
    
    set(ht,'FontSize',13,'FontWeight','bold','Color','blue');
end
...
function H=circle(center,radius)
...
X=X+center(1);
Y=Y+center(2);
H=patch(X,Y,'b-','LineWidth',1.5);
set(H,'FaceColor',[0.95,1,1])
...
if exist('ArSize') ~= 1,
    ArSize = 100 / 5000; %-- 10000 is an arbitrary value...
else
    ArSize = ArSize / 5000;
end
